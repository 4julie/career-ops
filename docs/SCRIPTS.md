...
Maps non-canonical statuses to their canonical equivalents and strips markdown bold and dates from the status column. Aliases like `Enviada` become `Applied`, `CERRADA` becomes `Discarded`, etc. Duplicate/repost info is moved to the notes column.
